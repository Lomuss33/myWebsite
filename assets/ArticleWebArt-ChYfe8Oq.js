...
import{c as Me,g as he,A as ce,_ as B}from"./index-CgJXkhs3.js";import{r as u,j as b}from"./react-vendor-D1LkQUJD.js";import"./swiper-BJPGAhIX.js";/* empty css              */import"./bootstrap-jfx39s8c.js";import"./vendor-BUjjXRU6.js";const Re=`<svg width="100%" height="100%" viewBox="0 0 750 500" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linejoin:round;stroke-miterlimit:2;">
...
`;function z(v,{timeoutMs:m=1200}={}){if(typeof window>"u")return v(),()=>{};if("requestIdleCallback"in window){const f=window.requestIdleCallback(()=>v(),{timeout:m});return()=>window.cancelIdleCallback(f)}const R=window.setTimeout(()=>v(),0);return()=>window.clearTimeout(R)}function Ce(v){if(!v)return{width:1,height:1};const m=v.getBoundingClientRect(),R=Math.max(1,Math.round(m.width||v.clientWidth||1)),f=Math.max(1,Math.round(m.height||v.clientHeight||1));return{width:R,height:f}}function $(v,m,R=1){var w;const{width:f,height:g}=Ce(v);(w=m==null?void 0:m.setSize)==null||w.call(m,f,g,R)}function le(v,m,R="smooth"){if(typeof window>"u")return;const f=document.getElementById(v),g=document.getElementById(`scrollable-${m}`);if(!f||!g)return;const w=f.getBoundingClientRect(),P=g.getBoundingClientRect(),a=g.scrollTop+(w.top-P.top);g.scrollTo({top:Math.max(0,a),behavior:R})}const Pe=9,ke=9,Se=10,Ie=["#0000ff","#008100","#ff1300","#000083","#810500","#2a9494","#000000","#808080"],Ne=["/images/web_art/patronus/bg.png","/images/web_art/patronus/layer-1.png","/images/web_art/patronus/layer-2.png","/images/web_art/patronus/layer-4.png","/images/web_art/patronus/layer-5.png","/images/web_art/patronus/layer-6.png"];function Ee(v=Pe,m=ke,R=Se){const f=v*m,g=Math.max(1,Math.min(R,f-1)),w=new Set;for(;w.size<g;)w.add(Math.floor(Math.random()*f));const P=new Array(f).fill(0);for(let a=0;a<f;a++){if(w.has(a)){P[a]=-1;continue}const C=a%m,M=Math.floor(a/m);let S=0;for(let x=-1;x<=1;x++)for(let s=-1;s<=1;s++){if(s===0&&x===0)continue;const t=C+s,e=M+x;t<0||e<0||t>=m||e>=v||w.has(e*m+t)&&(S+=1)}P[a]=S}return{rows:v,cols:m,mineCount:g,mines:w,counts:P}}function je(v,m,R,f){const g=new Set(R),w=[v];for(;w.length>0;){const P=w.pop();if(P==null||g.has(P)||f.has(P)||m.mines.has(P)||(g.add(P),m.counts[P]!==0))continue;const a=P%m.cols,C=Math.floor(P/m.cols);for(let M=-1;M<=1;M++)for(let S=-1;S<=1;S++){if(S===0&&M===0)continue;const x=a+S,s=C+M;x<0||s<0||x>=m.cols||s>=m.rows||w.push(s*m.cols+x)}}return g}function ue(v,m,R){const f=v.rows*v.cols-v.mineCount;if(m.size>=f)return!0;if(R.size!==v.mineCount)return!1;for(const g of v.mines)if(!R.has(g))return!1;return!0}function _e(v){return`Web art ${String(v||"tile").toLowerCase()} tile loading`}function Le({seed:v,reduceMotion:m}){const R=JSON.stringify(ge.split("<\/script>").join("<\\/script>")),f=JSON.stringify(v);return`<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
