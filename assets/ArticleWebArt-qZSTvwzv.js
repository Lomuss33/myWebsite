...
`;function $(w,{timeoutMs:p=1200}={}){if(typeof window>"u")return w(),()=>{};if("requestIdleCallback"in window){const d=window.requestIdleCallback(()=>w(),{timeout:p});return()=>window.cancelIdleCallback(d)}const R=window.setTimeout(()=>w(),0);return()=>window.clearTimeout(R)}function le(w){var a,C,g,k;if(!w)return{width:1,height:1};const p=w.getBoundingClientRect(),R=(C=(a=w.parentElement)==null?void 0:a.getBoundingClientRect)==null?void 0:C.call(a),d=(R==null?void 0:R.width)||((g=w.parentElement)==null?void 0:g.clientWidth)||1,M=(R==null?void 0:R.height)||((k=w.parentElement)==null?void 0:k.clientHeight)||d,x=Math.max(1,Math.round(p.width||w.clientWidth||d)),P=Math.max(1,Math.round(p.height||w.clientHeight||M));return{width:x,height:P}}function z(w,p,R=1){var a,C,g;const{width:d,height:M}=le(w),x=typeof window<"u"&&((C=(a=window.matchMedia)==null?void 0:a.call(window,"(pointer: coarse)"))==null?void 0:C.matches),P=Math.min(x?1:1.5,Math.max(1,Number(R)||1));if((d<32||M<32)&&typeof window<"u"){window.requestAnimationFrame(()=>{var v;const k=le(w);k.width>=32&&k.height>=32&&((v=p==null?void 0:p.setSize)==null||v.call(p,k.width,k.height,P))});return}(g=p==null?void 0:p.setSize)==null||g.call(p,d,M,P)}function ue(w,p,R="smooth"){if(typeof window>"u")return;const d=document.getElementById(w),M=document.getElementById(`scrollable-${p}`);if(!d||!M)return;const x=d.getBoundingClientRect(),P=M.getBoundingClientRect(),a=M.scrollTop+(x.top-P.top);M.scrollTo({top:Math.max(0,a),behavior:R})}const ke=9,Se=9,Ie=10,Ne=["#0000ff","#008100","#ff1300","#000083","#810500","#2a9494","#000000","#808080"],je=["/images/web_art/patronus/bg.png","/images/web_art/patronus/layer-1.png","/images/web_art/patronus/layer-2.png","/images/web_art/patronus/layer-4.png","/images/web_art/patronus/layer-5.png","/images/web_art/patronus/layer-6.png"];function Ee(w=ke,p=Se,R=Ie){const d=w*p,M=Math.max(1,Math.min(R,d-1)),x=new Set;for(;x.size<M;)x.add(Math.floor(Math.random()*d));const P=new Array(d).fill(0);for(let a=0;a<d;a++){if(x.has(a)){P[a]=-1;continue}const C=a%p,g=Math.floor(a/p);let k=0;for(let v=-1;v<=1;v++)for(let f=-1;f<=1;f++){if(f===0&&v===0)continue;const o=C+f,e=g+v;o<0||e<0||o>=p||e>=w||x.has(e*p+o)&&(k+=1)}P[a]=k}return{rows:w,cols:p,mineCount:M,mines:x,counts:P}}function _e(w,p,R,d){const M=new Set(R),x=[w];for(;x.length>0;){const P=x.pop();if(P==null||M.has(P)||d.has(P)||p.mines.has(P)||(M.add(P),p.counts[P]!==0))continue;const a=P%p.cols,C=Math.floor(P/p.cols);for(let g=-1;g<=1;g++)for(let k=-1;k<=1;k++){if(k===0&&g===0)continue;const v=a+k,f=C+g;v<0||f<0||v>=p.cols||f>=p.rows||x.push(f*p.cols+v)}}return M}function de(w,p,R){const d=w.rows*w.cols-w.mineCount;if(p.size>=d)return!0;if(R.size!==w.mineCount)return!1;for(const M of w.mines)if(!R.has(M))return!1;return!0}function Le(w){return`Web art ${String(w||"tile").toLowerCase()} tile loading`}function Te({seed:w,reduceMotion:p}){const R=JSON.stringify(Pe.split("<\/script>").join("<\\/script>")),d=JSON.stringify(w);return`<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
