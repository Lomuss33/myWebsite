import{a as Pe,p as Fe,P as Ae,W as _e,A as ke,S as Be,l as Ue,o as f,q as We,d as u,r as De,s as q,t as m,u as Ge,G as Ie,D as k,T as se,n as ce,I as Xe,v as ve,w as Ve,x as ue,C as Ye,y as je}from"./three.module-BHNaTqMO.js";const Ee=`
varying vec2  vUv;
...
`;function He(fe,me={}){const de=!!me.reduceMotion;let d=null,r=null,l=null,p=null,U=null,W=null,K=null,D=null,s=[],L=null,N=null,G=null,I=null,O=[],a=null,b=new ue,x=!1,M=null,S=1,C=1,Z=0,H=0,X=0,V=0,T=0,P=0,h=1,v=1,Y=0,F=0,j=0,g=0;function xe(){d=new Pe,d.fog=new Fe(0,.0045),r=new Ae(72,1,.1,1e3),r.position.set(0,0,50),l=new _e({canvas:fe,antialias:!0,powerPreference:"high-performance"}),l.toneMapping=ke,l.toneMappingExposure=.88,l.outputColorSpace=Be;const e=new Ue(13,13,260,80,160,!0);e.rotateX(Math.PI/2),e.translate(0,0,-55),p=new f({vertexShader:Ee,fragmentShader:Re,uniforms:{uTime:{value:0},uSpeed:{value:1},uWarpX:{value:0},uWarpY:{value:0},uBurst:{value:0}},side:We}),U=new u(e,p),d.add(U);const n=2600,i=new Float32Array(n*3),o=new Float32Array(n),c=new Float32Array(n),y=new Float32Array(n*3),ae=[[0,.85,1],[1,.15,.72],[.7,.3,1],[1,.75,.25],[.15,1,.75],[1,1,1]];for(let t=0;t<n;t++){const E=3+Math.random()*9.5,R=Math.random()*Math.PI*2;i[t*3+0]=Math.cos(R)*E,i[t*3+1]=Math.sin(R)*E,i[t*3+2]=(Math.random()-.5)*220,o[t]=.3+Math.random()*.9,c[t]=1.5+Math.random()*3.5;const _=ae[Math.floor(Math.random()*ae.length)];y[t*3+0]=_[0],y[t*3+1]=_[1],y[t*3+2]=_[2]}const A=new De;A.setAttribute("position",new q(i,3)),A.setAttribute("aSpeed",new q(o,1)),A.setAttribute("aSize",new q(c,1)),A.setAttribute("aColor",new q(y,3)),W=new f({vertexShader:Ne,fragmentShader:Oe,uniforms:{uTime:{value:0},uSpeed:{value:1}},transparent:!0,blending:m,depthWrite:!1,vertexColors:!0}),d.add(new Ge(A,W)),a=new Ie,a.position.set(0,0,-165),d.add(a);const ne=new f({vertexShader:B,fragmentShader:pe,uniforms:{uTime:{value:0}},side:k});K=new u(new se(9.5,.85,320,32,3,5),ne),a.add(K);const le=new f({vertexShader:B,fragmentShader:pe,uniforms:{uTime:{value:1.2}},side:k,transparent:!0,blending:m,depthWrite:!1,opacity:.45});D=new u(new se(12,.5,260,24,2,7),le),a.add(D);function z(t,E,R,_,Ce){const Te=new f({vertexShader:B,fragmentShader:qe,uniforms:{uTime:{value:0},uColor:{value:new Ye(R)}},transparent:!0,blending:m,depthWrite:!1,side:k}),$=new u(new je(t,E,8,160),Te);return $.rotation.x=_,$.rotation.y=Ce,$}s=[z(14,.1,55551,0,0),z(14,.1,16720072,Math.PI/2,0),z(14,.1,16762930,Math.PI/4,Math.PI/4),z(10.5,.08,6967551,0,Math.PI/3),z(17.5,.07,2162636,Math.PI/3,Math.PI/6),z(17.5,.07,16728160,Math.PI*.6,Math.PI/5)];for(const t of s)a.add(t);L=new f({vertexShader:B,fragmentShader:Ke,uniforms:{uTime:{value:0}},transparent:!0,blending:m,depthWrite:!1,side:k});const ie=new u(new ce(38,38),L);ie.position.z=-1,a.add(ie),G=new u(new Xe(5.5,1),new ve({color:11206655,wireframe:!0,blending:m,depthWrite:!1,transparent:!0,opacity:.5})),a.add(G),I=new u(new Ve(7.5,1),new ve({color:16744703,wireframe:!0,blending:m,depthWrite:!1,transparent:!0,opacity:.35})),a.add(I),N=new f({vertexShader:B,fragmentShader:Le,uniforms:{uTime:{value:0}},transparent:!0,blending:m,depthWrite:!1,side:k});const re=new u(new ce(80,80),N);re.position.z=-4,a.add(re),O=[ne,le,L,N,...s.map(t=>t.material)],b=new ue}function w(){const e=b.getElapsedTime();Math.min(b.getDelta(),.05),g*=.93;const n=h+g;v+=(n-v)*.06,Y*=.93,T+=(X*.012-T)*.08,P+=(V*.012-P)*.08,X*=.85,V*=.85,r.position.x=Z*.15,r.position.y=H*.15;const i=Math.max(0,v-1)*6;r.position.z=50-i+Math.sin(e*.22)*4,r.lookAt(0,0,0),p.uniforms.uTime.value=e,p.uniforms.uSpeed.value=Math.max(.1,v),p.uniforms.uWarpX.value=T,p.uniforms.uWarpY.value=P,p.uniforms.uBurst.value=Y,W.uniforms.uTime.value=e,W.uniforms.uSpeed.value=Math.max(.1,v),U&&(U.rotation.z=e*.04*v);for(let c=0;c<O.length;c++)O[c].uniforms.uTime.value=e+c*.15;F*=.92,j+=F;const o=.8+v*.5;a.rotation.z=j+e*.08*o,a.rotation.x=Math.sin(e*.14)*(.14+Math.abs(P)*.5),a.rotation.y=Math.cos(e*.1)*(.14+Math.abs(T)*.5),K.rotation.z=e*.28*o,D.rotation.z=-e*.18*o,D.rotation.x=e*.1,G.rotation.x=e*.5*o,G.rotation.y=e*.35,I.rotation.x=-e*.42*o,I.rotation.z=e*.28,s[0].rotation.z=e*.2*o,s[1].rotation.z=-e*.17*o,s[2].rotation.z=e*.13,s[3].rotation.z=-e*.25*o,s[4].rotation.z=e*.09,s[5].rotation.z=-e*.11*o,l.render(d,r)}function ee(){x&&(w(),M=requestAnimationFrame(ee))}function he(e,n,i=1){S=Math.max(1,Math.floor(e||1)),C=Math.max(1,Math.floor(n||1));const o=Math.max(1,Math.min(Number(i)||1,2));r.aspect=S/C,r.updateProjectionMatrix(),l.setPixelRatio(o),l.setSize(S,C,!1),w()}function ge(e,n,i=0,o=0){const c=e*S,y=n*C;Z=(c/S-.5)*14,H=(y/C-.5)*-9,X=i,V=o,x||w()}function oe(){Z=0,H=0}function J(){Y=1.2,h=3.5,window.clearTimeout(J.__timer),J.__timer=window.setTimeout(()=>{h=1},600)}function Q(){h=-1.5,window.clearTimeout(Q.__timer),Q.__timer=window.setTimeout(()=>{h=1},700)}function we(e){g+=e,g=Math.max(-2.5,Math.min(4,g))}function ye(e){F=e*.008,j+=F}function ze(){h=1,v=1,Y=0,X=0,V=0,T=0,P=0,F=0,j=0,g=0,oe(),x||w()}function be(){w()}function Me(){if(de){w();return}x||(b.start(),x=!0,M=requestAnimationFrame(ee))}function te(){x=!1,M!=null&&cancelAnimationFrame(M),M=null,b.stop()}function Se(){te(),l==null||l.dispose()}return xe(),{start:Me,stop:te,destroy:Se,reset:ze,renderStatic:be,setSize:he,setPointer:ge,clearPointer:oe,boost:J,reverseBurst:Q,addScrollBoost:we,drag:ye}}export{He as createTardisWormholeEngine};
